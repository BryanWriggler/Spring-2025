...
\begin{itemize}
    ...
    Lastly, if $f$ has only one zero and with multiplicity $1$, it must be irreducible: Suppose it's not irreducible, there eixsts noninvertible $g,h\in\mathcal{O}(\mathbb{C})$, with $f=gh$. But, since $g,h$ are not invertible, there exists $a,b\in\mathbb{C}$,
    ...
    (More precisely, all irreducible element is some associates of $(z-a)$ for som $a\in\mathbb{C}$).

    \hfil

    \textbf{Prime elements:}

    Since all prime elements are irreducible, they must be a subset of the irreducible elements; but in this case, we can prove that all irreducible elements are in fact prime.
    ...
    \begin{itemize}
        \item[$\implies$:] Suppose $f$ is invertible, then there existts $h\in\mathcal{O}(\mathbb{C})$, where $f(z)h(z)\equiv 1$. Hence, for all $a\in\mathbb{C}$, since $f(a)g(a)=1$, then $f(a)\neq 0$, showing that $f$ doesn't vanish for all $a\in\mathbb{C}$.
