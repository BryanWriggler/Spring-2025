...
\begin{columns}
    ...
    \block{Vector Fields \& Smooth Condition}{
        ...
        An equivalent condition of saying $X$ is smooth, is by smooth functions $f\in C^\infty(M)$: For all $u\in M$, $X(u)= X_u\in T_u(M)$ is a derivation at $u$, define $Xf:M\rightarrow\mathbb{R}$ by $Xf(u) = X_u(f)$, then $X$ is a smooth vector field iff $Xf\in C^\infty(M)$. Which, the \textbf{Derivation} is an equivalent condition for smooth vector field:
        \begin{theoremBox}
